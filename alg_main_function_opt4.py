...
from alg_function_opt2 import str2dict, merge_dict, pert_dict2str, pert_str2dict_list, remained_, isnotinfbl, propagate_inG, is_ssc, is_init_satisfied
...
def is_in_canalizedR(fixed_bycanal, fixed_desired, target_depc, logf):
    if fixed_bycanal!=fixed_desired:
        ...
        print('# ==== final Result: stabilized motif (Type1 (1.1)) ===== #', file = logf)
        ...
        print(fixed_bycanal, file = logf)
        print(fixed_desired, file = logf)
        print('# ==== final Result: stabilized motif (Type1 (1.2)) ===== #', file = logf)
...
def break_types(expG, considering_fbl, error, iter_n, logf):
    ...
    if error: 
        print('# ==== final Result: stabilized motif (Type2 (2.1)) ===== #', file = logf)
    ...
    if len(considering_fbl)>1000:
        print('# ==== final Result: stabilized motif (Type2 (2.2)) ===== #', file = logf)
        print('There are too many working fbls (>1000) at that condition', file = logf)
        logf.close()
        return True
    ...
    if initially_contradicted & is_splited & (iter_n>1):
        print('# ==== final Result: stabilized motif (Type0) ===== #', file = logf)
...
def perturbation_single(model_file, primes, pair, pert_i, pert_str, init_state, target_depc_ori, node_fbls):
    ...
    while 1:
        ...
        oscillating_motif = [x for x,y in remained_fbl_dict.items() if y['is_oscillating'][0]]
        considering_fbl = list(itertools.chain(*[x['ssc']+x['csm'] for x in remained_fbl_dict.values() if x['ssc']+x['csm'] != []]))
        if break_types(expG, considering_fbl, modality_error, iter_n, logf): return ('Wrong','-')

        print('# ==== Result1: fix the motif only by inital ===== #', file = logf)
        print(list(itertools.chain(*[x['ssc']+x['csm'] for x in remained_fbl_dict.values() if x['ssc']+x['csm'] != []])), file = logf)        
        ...
        def update_stabilzied_motif(remained_fbl_dict, remained_fbl_dict_info):
            ...
            while 1:
                ...
                for k,v in remained_fbl_dict.items():
                    ssc_fbl = remained_fbl_dict_info[k]['ssc']
                    csm_fbl = remained_fbl_dict_info[k]['csm']
                    
                    # (1) SSC
                    init_fixedssc = set(itertools.chain(*[list(itertools.chain(*[cn.split('&') for cn in ssc_ ])) for ssc_ in v['ssc']]))
                    if is_contradict(init_fixedssc):
                        print('# ==== final Result: stabilized motif (Type3 (3.1)) ===== #', file = logf)
                        print('There would be multiple steady states', file = logf)
                        logf.close()
                        return ('end') 
                    else:
                        fixed_dict_by_inits[k]['ssc'] = fixed_dict_by_inits[k]['ssc'] | set(v['ssc'])
                                
                    # (2) CSM  
                    for csmi, fbli in itertools.product(v['csm'], csm_fbl+ssc_fbl):               
                        ...
                        if (case1==True)&(case2==False)&(same==False): fixed_dict_by_inits[k]['csm'] = fixed_dict_by_inits[k]['csm'] | set([fbli]) 
                            ...
                            overlap_om = (set(csmi) & set(itertools.chain(*oscillating_motif)) != set()) & (set(fbli) & set(itertools.chain(*oscillating_motif)) != set())
                            fbl2 = set(sorted([x for x in csmi if '&' not in x])) | set(sorted([x for x in fbli if '&' not in x]))
                            if (overlap_om == True) & ~is_contradict(fbl2):
                                fixed_dict_by_inits[k]['csm'] = fixed_dict_by_inits[k]['csm'] | set([fbli]) | set([csmi])                                

                            elif (overlap_om == True) & is_contradict(fbl2):
                                print('# ==== final Result: stabilized motif (Type3 (3.2)) ===== #', file = logf)
                                print('There would be multiple steady states', file = logf)
                                logf.close()
                                print('Wrong, There would be multiple steady states') 
                                return ('end')                                     
                            else: 
                                print('# ==== final Result: stabilized motif (Type3 (3.3)) ===== #', file = logf)
                                print('There would be multiple steady states', file = logf)
                                logf.close()
                                print('Wrong, There would be multiple steady states')
                                return ('end') 
                             ...
                             continue
                    fixed_dict_by_inits[k]['ssc_stabilized'] = v['ssc_stabilized']
        ...
        fixed_dict_by_inits = {x:y for x,y in fixed_dict_by_inits.items() if np.any([y['ssc'] != set(), y['csm'] != set()])}
        fixed_ssc = set()
        init_csm = set()
        for values in fixed_dict_by_inits.values():
            fixed_ssc = fixed_ssc | values['ssc'] | set([x for x in values['csm'] if is_ssc(x)])
            init_csm = init_csm | (values['csm']-fixed_ssc)
               
        print('# ==== Result2: check the hierarchy btw motifs ===== #', file = logf)
        print(list(fixed_ssc | init_csm), file = logf)        


        # c-ii. Extend stabilized motif =========================================================================================
        # functional ssc & init_csm_updated 
        # INPUT: fixed_dict_by_inits
        check_ldoi = defaultdict(list)
        for fbl_cand in (fixed_ssc | init_csm) :
        ...
        # (1) update fixed_ssc
        remained_ssc = set(itertools.chain(*[v['ssc'] for v in remained_fbl_dict_info.values()])) - fixed_ssc        
        for ssc_cand in remained_ssc:
            if (set([x for x in ssc_cand if '&' not in x]) - (set(itertools.chain(*check_ldoi.values())) | set(init_str_p))) == set():
                fixed_ssc = fixed_ssc | set([ssc_cand])
                
        for fbl_cand in remained_ssc:
            _, fix_dict_reduced, _ = reduce_primes(logic_reduced_primes, merge_dict(pert_str2dict_list(itertools.chain(*[y.split('&') for y in fbl_cand])))) 
            check_ldoi[fbl_cand] = [pert_dict2str({x[0]:x[1]}) for x in fix_dict_reduced.items()]  

        
        remained_ssc -= fixed_ssc 
        functional_ssc = fixed_ssc
        # =======================================================
        all_sscfixed_ldoi = set(itertools.chain(*[check_ldoi[fbl_cand] for fbl_cand in fixed_ssc]))
        if is_contradict(all_sscfixed_ldoi):
            print('# ==== final Result: stabilized motif (Type3 (3.1)) ===== #', file = logf)
            print('There would be multiple steady states', file = logf)
            logf.close()
        ...
        if (time.time()-start) > 100000:
            print('time over')
            print('# ==== final Result: time over ===== #', file = logf)
            logf.close()
            return ('Wrong','-')
        ...
        for ki in fixed_dictfs.keys():
            ...
            fssc = set(vi['ssc'] + vi['csm']) & functional_ssc
            if fssc != set():
                fixed_dictfs[ki]['ssc'] = fssc
            ...
            for csm_cand in vi['csm']:     
                ...
                # 1) csm update
                all_ifixed_ldoi = set(itertools.chain(*[check_ldoi[fbl_cand] for fbl_cand in (fixed_ssc | init_csm - set([csm_cand]))]))                      
                if (set(itertools.chain(*[x.split('&') for x in csm_cand])) - all_ifixed_ldoi == set()):                   
                    fixed_dictfs[ki]['csm'] = fixed_dictfs[ki]['csm'] | set([csm_cand])
        

                # 2) functional_ssc in csm:                     
                sscfixed_ldoi = set(itertools.chain(*[check_ldoi[fbl_cand] for fbl_cand in fixed_ssc if not is_contradict(fbl_cand + csm_cand)]))
                compo_cand = [x.split('&') for x in csm_cand if '&' in x]
                
                # a. condpart_connected_by_ssc: condition part is connected by ssc
                condpart_connected_by_ssc = np.all([set(cond)-set(csm_cand)-sscfixed_ldoi == set() for cond in compo_cand])
                if condpart_connected_by_ssc == False: 
                    continue
                
                
                # b. condpart_fixed: by ssc or initial condition
                condpart_fixed1 = np.all([set(cond)-sscfixed_ldoi == set() for cond in [x.split('&') for x in csm_cand if '&' in x]])
                ...
                condition_fixed_case = condpart_connected_by_ssc & condpart_fixed & np.all(other_case) & (other_case != [])
                if condition_fixed_case: fssc_csm = fssc_csm | set([csm_cand])
   
                            
            fixed_dictfs[ki]['csm'] = fixed_dictfs[ki]['csm'] | fssc_csm
            functional_ssc = functional_ssc | fssc_csm


        # no update
        fixed_fs = {x:y for x,y in fixed_dictfs.items() if np.any([y['ssc'] != set(), y['csm'] != set()])}
        for ori_key in fixed_dictfs.keys():
            if ori_key not in fixed_fs.keys():
                fixed_fs[ori_key] = fixed_dict_by_inits[ori_key]
        
               
        print('# ==== Result3: update functional_ssc ===== #', file = logf)
        print(set(functional_ssc), file = logf)        
        


        all_funcsscfixed_ldoi = set(itertools.chain(*[check_ldoi[fbl_cand] for fbl_cand in functional_ssc]))
        if is_contradict(all_funcsscfixed_ldoi):
            print('# ==== final Result: stabilized motif (type4) ===== #', file = logf)
            print('There are multiple steady states (point)', file = logf)
            logf.close()
        ...
        for value_dict in fixed_fs.values():
            value_cand = value_dict['ssc'] | value_dict['csm']
            csm_cand_set = value_cand-functional_ssc
        ...
        final_fblstates = set(functional_ssc) | set(itertools.chain(*[x['ssc']| x['csm'] for x in fixed_fs.values() if (x['ssc'] | x['csm']) != set([])]))
        ...
        # fix!!
        fixbyfuncSSC = set(itertools.chain(*[check_ldoi[fbl_cand] for fbl_cand in functional_ssc]))
        fix_node = set([x if '~' not in x else x[1:] for x in fixbyfuncSSC])
        
        fixed_str = fixbyfuncSSC | set(itertools.chain(*[check_ldoi[final_fbls[fbl_cand]] for fbl_cand in set(final_fbls.keys()) if set(fbl_cand)&fix_node == set()]))        
        fixed_wi = pert_i.copy()
        fixed_wi.update(merge_dict(pert_str2dict_list(fixed_str))) 
        _, fixed_dict, _ = reduce_primes(primes, fixed_wi) 
        

        print('# ==== Result4: final fixed state ===== #', file = logf)
        ...
        if return_loop == True:
            ...
            pert_i.update(merge_dict(pert_str2dict_list(fixbyfuncSSC))) # wo csm info. 
            ...
            if logic_reduced == logic_reduced_c:
                ...
                print('# ==== final Result: stabilized nodes ===== #', file = logf)
