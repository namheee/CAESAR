...
import modules.BooleanDOI_DOI as BDOI
...
#%%
def solution_construction_score(G, Target, candidates, candidates_score):
    alpha = random.random()
    solution = []
    fail_in_construction = False
    while True:
...
def GRASP_target_control_customized(G, Target, max_itr, flagged, potential):
    initial_candidates=[]
    solutions=defaultdict(int)
    candidates_score = G.out_degree()

    for itr in range(max_itr):
        candidates=initial_candidates[:]
        #phase 1: construct a greedy randomized solution
        solution2, fail_in_construction=solution_construction_score(G,Target,candidates,candidates_score)
        if fail_in_construction:
            continue
        #append solution
        
        solutions[(tuple(sorted(solution2)),flagged[tuple(solution2)])]+=1
    return solutions,len(solutions)>0
